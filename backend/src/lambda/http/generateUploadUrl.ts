...
export const handler = middy(
  async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
    ...
    if (Object.keys(todoItem).length === 0) {
      logger.error(
        `user with id: ${userId} is attempting to generate upload url for a non-existent todo item with id ${todoId}`
      )
      return {
        statusCode: 400,
        body: JSON.stringify({
    ...
    if (todoItem.userId !== userId) {
      logger.error(
        `item with id ${todoId} was created by a different user and not by user with id ${userId}`
      )
      return {
        statusCode: 400,
        headers: {
